function [all_segments, penup_segments, pendown_segments, separators] = segment_by_strokes_vx_criterion(Word)
%% segment_by_strokes_vx_criterion
...
%  must have a continuous sign in the X speed.
%
% [ _all_segments_ , _penup_segments_ , _pendown_segments_ , _separators_ ] = _*segment_by_strokes_vx_criterion*_ ( _Word_ )
%
%%% Inputs
% 
...
%
    ...
    for k=1:length(all_segments_pen_up_down)
        [ ~, vx ,~ ] = compute_all_speeds(all_segments_pen_up_down(k));
        j = 1;
        ...
        while (j < length(vx)-1)
            ...
            while(j < length(vx)-1 && ( (sign(vx(j+1)) >= 0 && startSign   ==  1) || (sign(vx(j+1)) <= 0 && startSign   ==  -1) ))
                j         = j+1;
