...
%Plot PCA
subplot(2,1,1),
plot_principal_components_scatter( zFeatures, Labels ,labels_names , 1 , 2 );


%Plot LDA, Nb_of_axes = Nb_of_classes -1  
%Therefore our LDA has only 1 axe
subplot(2,1,2),
plot_linear_discriminant_analysis_scatter( zFeatures, Labels ,labels_names , 1 , 1 );


%Plot LDA Correlation Circle, Nb_of_axes = Nb_of_classes -1  
%Therefore our LDA has only 1 axe
figure(),
plot_correlation_circle_lda(zFeatures,Labels,features_names,1,1);

                           
    
figure(),
[ cross_valid_errors , model_coefficients ] = logistic_regression_l1_classifier_with_distinctive_features( zFeatures , Labels , features_names , labels_names );
