function [ Speed ] = compute_speed_unidimensional_histograms( Word , word_type , n_bins , delimiters_type , hist_type )
%% compute_speed_unidimensional_histograms
%  Computes all the histogram parameters on a word speed
...
%
%
% [ _Speed_ ] = _*compute_speed_unidimensional_histograms*_ ( _Word_ , _word_type_ , _n_bins_ , _delimiters_type_ , _hist_type_ )
%
%%% Inputs
% 
...
%%
    ...
    bin_loader = strcat('Bin_Delimiters_Speed_',word_type,'_',num2str(n_bins),'bins');
    eval(bin_loader);

    delimiters_x = eval(strcat('Bins.Speed.',word_type,'.X.Bins',num2str(n_bins),'.',delimiters_type));
    histogram_x  = a_good_histogram_function( vx_val , delimiters_x , hist_type );

    delimiters_y = eval(strcat('Bins.Speed.',word_type,'.Y.Bins',num2str(n_bins),'.',delimiters_type));
    histogram_y  = a_good_histogram_function( vy_val , delimiters_y , hist_type );

    delimiters_abs_x = eval(strcat('Bins.Speed.',word_type,'.absX.Bins',num2str(n_bins),'.',delimiters_type));
    histogram_abs_x  = a_good_histogram_function( abs_x , delimiters_abs_x , hist_type );

    delimiters_abs_y = eval(strcat('Bins.Speed.',word_type,'.absY.Bins',num2str(n_bins),'.',delimiters_type));
    histogram_abs_y  = a_good_histogram_function( abs_y , delimiters_abs_y , hist_type );

    delimiters_l2 = eval(strcat('Bins.Speed.',word_type,'.L2.Bins',num2str(n_bins),'.',delimiters_type));
    histogram_l2  = a_good_histogram_function( norm_l2 , delimiters_l2 , hist_type );

    delimiters_l1 = eval(strcat('Bins.Speed.',word_type,'.L1.Bins',num2str(n_bins),'.',delimiters_type));
    histogram_l1  = a_good_histogram_function( norm_l1 , delimiters_l1 , hist_type );

    delimiters_linf = eval(strcat('Bins.Speed.',word_type,'.Li.Bins',num2str(n_bins),'.',delimiters_type));
    histogram_linf  = a_good_histogram_function( norm_linf , delimiters_linf , hist_type );

    delimiters_linfn = eval(strcat('Bins.Speed.',word_type,'.Lin.Bins',num2str(n_bins),'.',delimiters_type));
    histogram_linfn  = a_good_histogram_function( norm_linfn , delimiters_linfn , hist_type );

    %Mean Values for each norm
    Speed.L2    = histogram_l2;
    Speed.L1    = histogram_l1;
    Speed.Linf  = histogram_linf;
    Speed.Linfn = histogram_linfn;
    Speed.Xabs  = histogram_abs_x;
    Speed.Yabs  = histogram_abs_y;
    Speed.X     = histogram_x;
    Speed.Y     = histogram_y;
