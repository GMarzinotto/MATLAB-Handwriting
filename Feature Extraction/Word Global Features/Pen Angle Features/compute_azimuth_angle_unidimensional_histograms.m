function [ Azimuth_Angle ] = compute_azimuth_angle_unidimensional_histograms( Word , word_type , n_bins , delimiters_type , hist_type )
%% compute_azimuth_angle_unidimensional_histograms
%  Computes all the unidimensional histogram on azimuth angle
...
%
%
% [ _Azimuth_Angle_ ] = _*compute_azimuth_angle_unidimensional_histograms*_ ( _Word_ , _word_type_ , _n_bins_ , _delimiters_type_ , _hist_type_ )
%
%%% Inputs
% 
...
%%
    ...
    bin_loader = strcat('Bin_Delimiters_Azimuth_',word_type,'_',num2str(n_bins),'bins');
    eval(bin_loader);
    
    
    delimiters = eval(strcat('Bins.Azimuth.',word_type,'.Bins',num2str(n_bins),'.',delimiters_type));
    
    if( ~isempty(iWord) ) 
        Azimuth_Angle  = a_good_histogram_function( iWord.Az , delimiters , hist_type );
    else
        Azimuth_Angle  = a_good_histogram_function( [] , delimiters , hist_type );    
    end
