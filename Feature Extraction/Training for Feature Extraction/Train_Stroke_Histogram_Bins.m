...
feature_to_bin      = {'Stroke_Time_Durations','Stroke_Lengths'}; 
tracking_type       = {'ON_TABLET','ON_AIR','ON_TABLET_ON_AIR'};
stroke_criterion    = {'PENUP_CRITERION','VX_CRITERION','VY_CRITERION','VX_VY_CRITERION'};
bin_numbers         = [3:5];
...
for i=1:length(stroke_criterion)
    
    istroke_criterion = stroke_criterion{i};
    ...
    for j=1:length(tracking_type)
        ...
        for k=1:length(feature_to_bin)
            ...
            feature_name  = strcat(feature_to_bin{k},'_',istroke_criterion);
            ...
            for l=1:nWords
               ...
               switch(kfeature_name)
                   case 'Stroke_Lengths'
                       [ ~ , x_val , y_val ] = compute_strokes_lengths( lWord , jtracking_type , istroke_criterion );
                       use_x = true;
                       use_y = true;
                       use_xabs = true;
                       use_yabs = true;
                   ...
                   case 'Stroke_Time_Durations'
                       unidim_val = compute_strokes_time_durations( lWord , jtracking_type , istroke_criterion );
