...
feature_to_bin      = {'Pressure','Pressure_T_Derivative','Pressure_XY_Derivative'}; 
tracking_type       = {'ON_TABLET','ON_TABLET_ON_AIR'};
bin_numbers         = [4];
    ...
    for k=1:length(feature_to_bin)
        ...
        for l=1:nWords
           ...
           switch(kfeature_to_bin)
               ...
               case 'Pressure_T_Derivative'
                   [ unidim_val , ~ , ~ ] = compute_valid_pressure_derivatives( lWord );
                   is_unidimensional = true;
               case 'Pressure_XY_Derivative'
                   [ ~ , x_val , y_val ] = compute_valid_pressure_derivatives( lWord );
                   use_x = true;
                   use_y = true;
                   use_xabs = true;
                   use_yabs = true;
