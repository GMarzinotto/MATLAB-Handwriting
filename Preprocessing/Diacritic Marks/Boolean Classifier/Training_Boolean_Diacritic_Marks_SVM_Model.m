        ...
        if(isfield(iWriter,strcat('Test',num2str(j))))
            ...
            if(isfield(jTest,'strokes_diacritic_marks') )
                ...
                for k=1:length(pendown_segments)
                    ...
                    all_labels      		= [ all_labels; (~strcmp(jTest.strokes_diacritic_marks{k},'normal')) ];
...
SVM_diacritic_marks_boolean_model = fitcecoc( all_features ,all_labels,'ClassNames',class_names,'Learners',t,'FitPosterior',1);
                         
CVMdl = crossval(SVM_diacritic_marks_boolean_model);
