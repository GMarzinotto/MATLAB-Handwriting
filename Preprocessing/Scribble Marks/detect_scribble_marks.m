function [  labels_per_stroke , labels_per_sample  ] = detect_scribble_marks( Word )
    ...
    for j=1:length(Line_List)
        ...
        for k=1:length(pendown_segments)
            features  = compute_scribble_marks_features_for_classifier( pendown_segments(k) , Canonical_Lines );
            all_features = [ all_features; features ];
        end

    end
    
    load('SVM_scribble_marks_model.mat')
    
    
    [labels_on_tablet , NegLoss , PBScore , Posterior ] = predict(SVM_scribble_marks_model,all_features);
