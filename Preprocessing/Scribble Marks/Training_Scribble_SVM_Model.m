            ...
            if(isfield(jTest,'strokes_punctuation_marks'))
                ...
                for k=1:length(pendown_segments)
                    ...
                    if(strcmp(jTest.strokes_diacritic_marks(k),'normal'))
                        ...
                        [ features , feature_names ] = compute_scribble_marks_features_for_classifier( pendown_segments(k) , Canonical_Lines );

                        all_features            = [ all_features; features ];
                        all_labels      		= [ all_labels; jTest.is_scribble_marks(k) ];
...
SVM_scribble_marks_model = fitcecoc( all_features ,all_labels,'ClassNames',class_names,'Learners',t,'FitPosterior',1);
                         
CVMdl = crossval(SVM_scribble_marks_model);
...
figure(),
plot_sne_components_scatter( all_features , all_labels_num , {'normal','scribble'} )
